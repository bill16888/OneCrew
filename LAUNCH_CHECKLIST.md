...
| `NEXTAUTH_URL`                     | 非空，有公网 https URL                      | 例：`https://onecrew.up.railway.app`                      |
...
| `SEED_EMAIL_DOMAIN`                | 种子用户邮箱域名（如 `workspace.local`）   | **生产 seed 必填**——未设时拒绝种入 `onecrew.local` |
...
```
Seeding workspace "OneCrew Demo Workspace" (id=ws_default)…
✓ Workspace ws_default (OneCrew Demo Workspace)
✓ Human user mia@onecrew.local
✓ Human user dev@onecrew.local
✓ Human user pm@onecrew.local
...
打开 https://<your-domain>/login，用 `mia@onecrew.local` / `password123` 登录。登录成功后浏览器会自动 connect socket.io。
...
> 每一条都用浏览器 `mia@onecrew.local` / `password123` 登录后做。
